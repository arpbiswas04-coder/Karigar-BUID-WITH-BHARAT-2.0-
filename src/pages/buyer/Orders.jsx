...
export default function Orders(){
 ...
 return <section className="premium-section"><h1>Your Orders</h1><p>Orders placed with your patron account. Checkout currently uses test payments.</p>{data.token!==token?<p>Loading orders...</p>:data.error?<p role="alert">{data.error}</p>:!orders.length?<p>No orders yet. <Link to="/collections">Browse crafts</Link></p>:orders.map(o=><article key={o.id} className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 my-4"><h2>{o.orderNumber}</h2><p>{new Date(o.createdAt).toLocaleDateString()} &middot; {o.status.replaceAll('_',' ')} &middot; {formatCurrency(o.totalAmount)}</p><p>Escrow: {o.escrow?.status||'Not available'}</p>{o.items.map(item=><div key={item.id} className="py-3 border-b border-outline-variant"><Link to={`/product/${item.productId}`}>{item.product.title}</Link><p>{item.product.artisan.fullName} &middot; {item.quantity} units &middot; {formatCurrency(item.unitPrice)}</p><Link to={`/product/${item.productId}`}>View product / write a review</Link></div>)}</article>)}</section>;
}
