...
const filePath = path.join(projectRoot, 'src/constants/culturalTranslations.js');
...
export function translateCollectionDescription(desc, item = {}, lang = 'en') {
  ...
  // Synthesized artisan pieces format:
...
export function translateProductSpec(specValue, lang = 'en') {
  if (!specValue || typeof specValue !== 'string') return specValue || '';
  const code = (lang || 'en').split('-')[0].toLowerCase();
  if (code === 'en') return specValue;

  const specMap = {
    "Craft Standard Dimensions": { "hi": "शिल्प मानक आयाम", "bn": "কারুশিল্পের আদর্শ মাপ" },
    "Locally sourced authentic materials": { "hi": "स्थानीय रूप से प्राप्त प्रामाणिक सामग्री", "bn": "স্থানীয়ভাবে সংগৃহীত খাঁটি উপকরণ" },
    "Handcrafted traditional technique": { "hi": "पारंपरिक हस्तशिल्प तकनीक", "bn": "হাতে তৈরি ঐতিহ্যবাহী কৌশল" },
    "Handmade by master artisan": { "hi": "कुशल शिल्पकार द्वारा हस्तनिर्मित", "bn": "দক্ষ কারিগর দ্বারা হাতে তৈরি" },
    "Natural artisanal process": { "hi": "प्राकृतिक शिल्प प्रक्रिया", "bn": "প্রাকৃতিক কারুশিল্প পদ্ধতি" },
    "Organic Mineral Dyes": { "hi": "जैविक खनिज रंग", "bn": "জৈব খনিজ রঙ" },
    "100% Wild Forest Tussar Silk": { "hi": "१००% प्राकृतिक जंगली टसर रेशम", "bn": "১০০% বুনো তসর রেশম" }
  };

  if (specMap[specValue.trim()]?.[code]) {
    return specMap[specValue.trim()][code];
  }

  return transliterateText(specValue, code);
}
`;

if (!fileContent.includes('export const COLLECTION_TITLE_TRANSLATIONS')) {
  fileContent += '\n' + codeToAppend;
  fs.writeFileSync(filePath, fileContent, 'utf8');
  console.log('Appended collection title and description translation helpers to culturalTranslations.js');
} else {
  console.log('Helpers already exist in culturalTranslations.js');
}
